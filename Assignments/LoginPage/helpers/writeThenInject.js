...
module.exports.thisReport = (report) => {
    fs.writeFileSync('./report.json', JSON.stringify(report, undefined, 2), (err) => { //  Overwrite report.json with the updated report
        if(err) {
    ...
    for(keys in report) {
        ...
        thisInject = thisInject + `<tr><td>${name}</td><td>${keys}</td><td>${dept}</td><td><form action='/update/${keys}' method='post'><input type='submit' value='Delete'></form></td><tr>`;
    }
    return thisInject;
};